...
# Function to create database connectionclear
...
def on_message(client, userdata, msg):
    ...
    if len(Sensor_Data) >= 2:
        sensor_id, timestamp = Sensor_Data[:2]
        formatted_timestamp = datetime.datetime.strptime(timestamp, "%Y-%m-%dT%H:%M:%S").strftime('%Y-%m-%d %H:%M:%S')
        query = f"""
        INSERT INTO Sensor_Status (SENSORID, TIMESTAMP)
        VALUES ('{sensor_id}', '{formatted_timestamp}')
        ON DUPLICATE KEY UPDATE Timestamp='{formatted_timestamp}'
        """
